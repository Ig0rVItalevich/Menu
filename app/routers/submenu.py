...
from structs.submenu import *

submenuRouter = APIRouter()


@submenuRouter.get(path="/",
...
@submenuRouter.get(path="/{submenu_id}",
                   summary="Get submenu",
                   description="Get concrete submenus",
                   response_model=SubMenuShow,
                   status_code=status.HTTP_200_OK)
def getSubmenu(menu_id: int = Path(..., gt=0), submenu_id: int = Path(..., gt=0)):
...
def createSubmenu(submenu: SubMenuCreate, menu_id: int = Path(..., gt=0)):
    ...
    return SubMenuCreated(id=submenuCreated.id, title=submenuCreated.title,
                                  description=submenuCreated.description)


@submenuRouter.patch(path="/{submenu_id}",
                     summary="Upgrade submenu",
                     description="Upgrade one submenu",
                     response_model=SubMenuCreated,
                     status_code=status.HTTP_200_OK)
def updateSubmenu(submenu: SubMenuCreate, menu_id: int = Path(..., gt=0), submenu_id: int = Path(..., gt=0)):
...
@submenuRouter.delete(path="/{submenu_id}",
                      summary="Delete submenu",
                      description="Delete one submenu",
                      status_code=status.HTTP_200_OK)
def deleteSubmenu(menu_id: int = Path(..., gt=0), submenu_id: int = Path(..., gt=0)):
